...
CUfunction CudaPlatform::load_kernel(DeviceId dev, const std::string& file, const std::string& kernelname) {
    ...
    if (mod_it == mod_cache.end()) {
        ...
        std::string ptx_path = file + ".ptx";

        struct stat ptx_attrib, src_attrib;
        if (!stat(ptx_path.c_str(), &ptx_attrib) &&
            !stat(file.c_str(), &src_attrib)     &&
            src_attrib.st_mtime <= ptx_attrib.st_mtime) {
            debug("Using cached PTX file '%'", ptx_path);
            // Do not cache the PTX code because we already use the cached version
            cache_ptx = false;
            src_path  = ptx_path;
            ptx = load_file(ptx_path);
        } else if (ext == "ptx" && (std::ifstream(file).good() || files_.count(file))) {
            ptx = load_file(file);
        } else if (ext == "cu" && (std::ifstream(file).good() || files_.count(file))) {
            ptx = compile_cuda(dev, file, load_file(file));
        } else if (ext == "nvvm" && (std::ifstream(file).good() || files_.count(file))) {
            ptx = compile_nvptx(dev, file, load_file(file));
        } else if (ext == "nvvm" && (std::ifstream(file + ".bc").good() || files_.count(file))) {
            // Do not cache the PTX code generated by the driver. There is otherwise an
            // ambiguity if both the .nvvm and .nvvm.bc files are present. Besides,
            // compilation times are rather fast using the driver.
            cache_ptx = false;
            src_path += ".bc";
            ptx = compile_nvvm(dev, src_path, load_file(src_path));
        } else {
...
#endif
std::string CudaPlatform::compile_cuda(DeviceId dev, const std::string& filename, const std::string& program_string) const {
    CUjit_target target_cc = devices_[dev].compute_capability;
    #if CUDA_VERSION < 9000
